...
function nodeModulesPaths(basedir, options) {
  ...
  const physicalBasedir = (0, (_realpathNative || _load_realpathNative()).sync)(basedirAbs);

  const paths = [physicalBasedir];
  let parsed = (_path || _load_path()).default.parse(physicalBasedir);
  while (parsed.dir !== paths[paths.length - 1]) {
    const realParsedDir = (0, (_realpathNative || _load_realpathNative()).sync)(parsed.dir);
    paths.push(realParsedDir);
    parsed = (_path || _load_path()).default.parse(realParsedDir);
  }
