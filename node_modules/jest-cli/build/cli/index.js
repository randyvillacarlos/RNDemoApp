...
const ensureNoDuplicateConfigs = (parsedConfigs, projects, rootConfigPath) => {
  ...
  for (const config of parsedConfigs) {
    ...
    if (configPathMap.has(configPath)) {
      const message = `Whoops! Two projects resolved to the same config path: ${(_chalk || _load_chalk()).default.bold(String(configPath))}:

  Project 1: ${(_chalk || _load_chalk()).default.bold(projects[parsedConfigs.findIndex(x => x === config)])}
  Project 2: ${(_chalk || _load_chalk()).default.bold(projects[parsedConfigs.findIndex(x => x === configPathMap.get(configPath))])}

This usually means that your ${(_chalk || _load_chalk()).default.bold('"projects"')} config includes a directory that doesn't have any configuration recognizable by Jest. Please fix it.
`;
...
const _run = (() => {
  var _ref8 = _asyncToGenerator(function* (globalConfig, configs, hasDeprecationWarnings, outputStream, onComplete) {
    ...
    var _ref9 = yield buildContextsAndHasteMaps(configs, globalConfig, outputStream);
...
const runWatch = (() => {
  var _ref10 = _asyncToGenerator(function* (contexts, configs, hasDeprecationWarnings, globalConfig, outputStream, hasteMapInstances, changedFilesPromise) {
    if (hasDeprecationWarnings) {
      try {
...
const runWithoutWatch = (() => {
  var _ref11 = _asyncToGenerator(function* (globalConfig, contexts, outputStream, onComplete, changedFilesPromise) {
    const startRun = (() => {
